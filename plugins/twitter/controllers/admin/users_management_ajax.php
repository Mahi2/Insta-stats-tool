...
$result = $database->query("
	SELECT
		...
		(SELECT COUNT(*) FROM `twitter_users`) AS `total_before_filter`,
		(SELECT COUNT(*) FROM `twitter_users` WHERE `username` LIKE '%{$datatable->get_search()}%' OR `full_name` LIKE '%{$datatable->get_search()}%') AS `total_after_filter`
	FROM
		`twitter_users`
	WHERE
...
while($entry = $result->fetch_object()) {
    ...
    if($entry->is_private) {
        $username_extra .= '<span class="text-muted" data-toggle="tooltip" title="' . $language->twitter->admin_users_management->tooltip->private . '"><i class="fa fa-lock fa-sm"></i></span>&nbsp;';
    }

    if($entry->is_verified) {
        $username_extra .= '<span class="text-muted" data-toggle="tooltip" title="' . $language->twitter->admin_users_management->tooltip->verified . '"><i class="fa fa-check-circle fa-sm"></i></span>&nbsp;';
    }

    if($entry->is_demo) {
        $username_extra .= '<span class="text-muted" data-toggle="tooltip" title="' . $language->twitter->admin_users_management->tooltip->demo . '"><i class="fa fa-adjust fa-sm"></i></span>&nbsp;';
    }

    $entry->username = '<a href="' . $settings->url . 'report/' . $entry->username . '/twitter"  target="_blank">' . $entry->username . '</a> ' . $username_extra;
