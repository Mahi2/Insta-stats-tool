    ...
    public function getAccount($username)
    {
        ...
        if (isset($userArray['entry_data']['LoginAndSignupPage'])) {
            throw new InstagramException(
                'Instagram blocked this request and rate limiting occured.',
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
     ...
     * @throws InstagramNotFoundException
     */
    public function getPaginateMedias($username, $maxId = '', $account = false)
    {
        $account = $account ? $account : $this->getAccount($username);
